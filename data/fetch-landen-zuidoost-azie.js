#!/usr/bin/env node
// Issue #55 (set 8.7 Zuidoost-Azië): Natural Earth 1:10m
// admin_0_countries → landen-zuidoost-azie.geojson.
//
// 9 landen: Myanmar, Thailand, Vietnam, Cambodja, Laos, Maleisië, Singapore,
// Indonesië, Filipijnen. Indonesië strekt van Sumatra (~95°E) tot Papoea
// (~141°E) met Sumba-eilanden tot ~-11°S; Filipijnen tot ~21°N.
...
const LANDEN_PATH = path.join(__dirname, '..', 'landen-zuidoost-azie.geojson');
const CACHE = path.join(__dirname, 'natural-earth', 'ne_10m_admin_0_countries.geojson');

// Bbox: Sumatra-west (~95°E) tot Papoea-oost (~142°E); Sumba/Timor (~-11°S)
// tot Myanmar/Filipijnen-noord (~28°N). Alle ringen erbuiten weggooien.
const ZOA_BBOX = { minLon: 92, maxLon: 142, minLat: -12, maxLat: 29 };

// eps in graden voor RDP. Indonesië/Filipijnen hebben veel eilanden → iets
// hogere eps, anders wordt het bestand te groot. Singapore heel klein → laag.
const COUNTRIES = {
  Q836: { nl: 'Myanmar',    eps: 0.025 },
  Q869: { nl: 'Thailand',   eps: 0.025 },
  Q881: { nl: 'Vietnam',    eps: 0.020 },
  Q424: { nl: 'Cambodja',   eps: 0.020 },
  Q819: { nl: 'Laos',       eps: 0.020 },
  Q833: { nl: 'Maleisië',   eps: 0.025 },
  Q334: { nl: 'Singapore',  eps: 0.005 },
  Q252: { nl: 'Indonesië',  eps: 0.035 },
  Q928: { nl: 'Filipijnen', eps: 0.025 },
};
...
function processFeature(qid, neFeat) {
  ...
  rings = rings.filter(r => overlapsBBox(r, ZOA_BBOX));
...
(async () => {
  ...
  console.log(`\nMatching ${Object.keys(COUNTRIES).length} ZOA-landen uit ${ne.features.length} NE-features...\n`);
  ...
  for (const feat of ne.features) {
    ...
    try {
      ...
      console.log(`  ${nl.padEnd(12)} ${geometry.type.padEnd(14)} ${rings} ring${rings>1?'en':''}, ${String(pts).padStart(5)} pts`);
      landen.features.push({
        type: 'Feature',
        properties: { name: nl, sets: [87] },
        geometry,
      });
    } catch (e) {
  ...
  console.log(`\n  ✓ landen-zuidoost-azie.geojson: ${landen.features.length} features, ${Math.round(size/1024)} KB`);
})();
