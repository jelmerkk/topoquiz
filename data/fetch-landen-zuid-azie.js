#!/usr/bin/env node
// Issue #53 (set 8.5 Zuid-Azië): Natural Earth 1:10m
// admin_0_countries → landen-zuid-azie.geojson.
// Zelfde pipeline als fetch-landen-midden-oosten.js.
...
const LANDEN_PATH = path.join(__dirname, '..', 'landen-zuid-azie.geojson');
const CACHE = path.join(__dirname, 'natural-earth', 'ne_10m_admin_0_countries.geojson');

// Bbox: Sri Lanka zuid (~5°N) tot Kazachstan noord (~56°N),
// Kaspische west (~46°E) tot Bangladesh oost (~93°E).
const ZA_BBOX = { minLon: 45, maxLon: 95, minLat: 5, maxLat: 56 };

// eps in graden voor RDP — groter land = hogere eps.
const COUNTRIES = {
  Q232: { nl: 'Kazachstan',  eps: 0.040 },
  Q265: { nl: 'Oezbekistan', eps: 0.030 },
  Q889: { nl: 'Afghanistan', eps: 0.025 },
  Q843: { nl: 'Pakistan',    eps: 0.030 },
  Q668: { nl: 'India',       eps: 0.035 },
  Q837: { nl: 'Nepal',       eps: 0.015 },
  Q902: { nl: 'Bangladesh',  eps: 0.015 },
  Q854: { nl: 'Sri Lanka',   eps: 0.015 },
};
...
function processFeature(qid, neFeat) {
  ...
  // Neem alle ringen die binnen de bbox vallen — India heeft Andamanen,
  // Lakshadweep; Bangladesh enclaves; Pakistan kuststroken.
  let rings = polys.map(p => p[0]);
  rings = rings.filter(r => overlapsBBox(r, ZA_BBOX));
...
(async () => {
  ...
  console.log(`\nMatching ${Object.keys(COUNTRIES).length} ZA-landen uit ${ne.features.length} NE-features...\n`);
  ...
  console.log(`\n  ✓ landen-zuid-azie.geojson: ${landen.features.length} features, ${Math.round(size/1024)} KB`);
})();
