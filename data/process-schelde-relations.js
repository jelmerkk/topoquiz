#!/usr/bin/env node
// Chains + RDP-simplifies outer ways from Oosterschelde and Westerschelde relations.
// Outputs: data/overpass/oosterschelde-processed.json, westerschelde-processed.json
...
function process(name, relFile, eps) {
  console.log(`\n── ${name} ──`);
  const raw = JSON.parse(fs.readFileSync(path.join(DIR, relFile), 'utf8'));
  ...
  console.log(`${outerWays.length} outer ways, ${innerWays.length} inner ways (islands/land — ignored)`);
  ...
  console.log(`simplified (eps=${eps}): ${simplified.length} pts`);
  ...
  const outFile = path.join(DIR, name.toLowerCase() + '-processed.json');
  fs.writeFileSync(outFile, JSON.stringify(simplified, null, 2));
  console.log(`Saved to data/overpass/${name.toLowerCase()}-processed.json`);
  return simplified;
}

process('oosterschelde', 'oosterschelde-relation.json', 0.002);
process('westerschelde', 'westerschelde-relation.json', 0.002);
