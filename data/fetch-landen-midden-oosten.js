#!/usr/bin/env node
// Issue #52 (set 8.4 Midden-Oosten): Natural Earth 1:10m
// admin_0_countries → landen-midden-oosten.geojson.
// Zelfde pipeline als fetch-landen-noord-midden-amerika.js.
...
const LANDEN_PATH = path.join(__dirname, '..', 'landen-midden-oosten.geojson');
const CACHE = path.join(__dirname, 'natural-earth', 'ne_10m_admin_0_countries.geojson');

// Bbox: Jemen zuid (~12°N) tot Georgië noord (~43.5°N),
// Turkije west (~26°E) tot Iran oost (~63°E).
const ME_BBOX = { minLon: 25, maxLon: 65, minLat: 10, maxLat: 45 };

// eps in graden voor RDP — groter land = hogere eps.
const COUNTRIES = {
  Q43:  { nl: 'Turkije',        eps: 0.030 },
  Q858: { nl: 'Syrië',          eps: 0.020 },
  Q822: { nl: 'Libanon',        eps: 0.015 },
  Q801: { nl: 'Israël',         eps: 0.015 },
  Q810: { nl: 'Jordanië',       eps: 0.020 },
  Q796: { nl: 'Irak',           eps: 0.025 },
  Q794: { nl: 'Iran',           eps: 0.040 },
  Q851: { nl: 'Saoedi-Arabië',  eps: 0.040 },
  Q805: { nl: 'Jemen',          eps: 0.025 },
  Q817: { nl: 'Koeweit',        eps: 0.015 },
  Q230: { nl: 'Georgië',        eps: 0.020 },
  Q399: { nl: 'Armenië',        eps: 0.015 },
  Q227: { nl: 'Azerbeidzjan',   eps: 0.020 },
};
...
function processFeature(qid, neFeat) {
  ...
  // Neem alle ringen die binnen de bbox vallen (niet alleen de hoofdring,
  // want Azerbeidzjan heeft Nachitsjevan als exclave, Turkije heeft
  // Europees Thracië + Anatolisch deel, enz.)
  let rings = polys.map(p => p[0]);
  rings = rings.filter(r => overlapsBBox(r, ME_BBOX));
...
(async () => {
  ...
  console.log(`\nMatching ${Object.keys(COUNTRIES).length} ME-landen uit ${ne.features.length} NE-features...\n`);
  ...
  console.log(`\n  ✓ landen-midden-oosten.geojson: ${landen.features.length} features, ${Math.round(size/1024)} KB`);
})();
